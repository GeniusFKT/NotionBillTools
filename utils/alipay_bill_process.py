...
# 根据时间获取文件名
# file_name = "支付宝支付账单(%s-%s).csv" % (start, end)
file_name = "支付宝支付账单(%s-%s).csv" % ("20210701", "20210731")
file_path = os.path.join(BILL_PATH, file_name)


def get_alipay_bill(database_id: str):
    with open(file_path, "r", encoding="gbk", newline="") as f:
        ...
        for row in csv_reader:
            ...
            start_date = datetime.datetime.fromisoformat(row.get("付款时间                ")[:-1])
